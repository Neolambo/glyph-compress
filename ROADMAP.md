...
- [ ] **Open, measured, not fixed — routing relevance.** Against six queries with unambiguous ground truth the router retrieved the right file **0/6**, before and after the staleness fix. Two causes: relevance scores `path + owner + symbols + imports` and never file content (`src/dashboard.js` ranks 4th for a query about a function defined inside it), and `usageBoost()` reinforces whatever `routeAndCompress()` selected before, with no correctness signal — a feedback loop on its own output. Needs content indexing and a scoring redesign, not a patch.
