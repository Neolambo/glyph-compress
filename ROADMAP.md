...
- [x] Current stable release is `v1.32.1`: fixed the *other half* of v1.32.0's `level: 'auto'` trust-policy bug. v1.32.0 scoped its fix to `compressText()` and left `compressMessages()` alone; the same root cause was live there and far worse — **325 vs 4922 tokens (15x)** on a realistic 4-message thread — in the path the proxy, `wrapOpenAI`/`wrapAnthropic`, and the VS Code extension all use. `trustPolicy` is now captured/restored alongside the level, since the two are coupled.
...
- [x] Root npm package, VS Code extension manifest, VS Code extension lockfile, and `server.json` are versioned to `1.32.1`.
- [x] npm `latest` is caught up to `1.31.1` (published directly by the maintainer, working around the OTP-confirmation flow this environment can't complete non-interactively); `1.24.0`/`1.25.0`/`1.27.0` were left unpublished as a harmless historical gap since `latest` already carries their fixes. `v1.32.1` publication is pending the same maintainer-driven `npm publish` step. VS Code Marketplace is caught up to `v1.31.1`, published directly by the maintainer through the web management portal (marketplace.visualstudio.com/manage) rather than the CLI/PAT flow; `v1.32.1` publication there is still pending.
- [x] `npm test` passes 27 suites (unit, CLI, workspace, extension, proxy, metadata, snapshot, integration, holographic, intent, codebook-completeness, auto-level, cache-prefix-stability, tech-glyph-economics, code-minify-economics, context-router, mcp-server, team-codebook, logger, ast-spans, trust-warnings, npm-pack-smoke, adaptive-workspace-memory, anthropic-bridge, token-estimator-accuracy, context-budget-planner) for `v1.32.1`. `test/benchmark-alternatives.js` and the tokenizer-calibration/comprehension scripts remain deliberately excluded — reporting/manual tools, not pass/fail regression gates.
