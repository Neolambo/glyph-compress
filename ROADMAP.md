...
- [ ] **Open — routing relevance, second half. The remaining 3/6 all fail the same way: the test file outranks the source file.** `test/privacy-redaction.js` beats `vscode-ext/glyph-middleware.js` for "privacy redaction pattern for api keys"; `test/token-estimator-accuracy.js` beats `src/token-estimator.js`. This is not the usage bug fixed in v1.33.3 — it is the second cause, and it needs real work rather than a weight change:
  ...
  - **Next:** content indexing (BM25 over file text, not metadata). That is a real piece of work, not a patch, and it changes the codebook format.
  - **Refuted along the way, recorded so it is not retried:** IDF term weighting. Rare terms outweighing common ones sounds obviously right and measured as nothing — retrieval 3/6 and noise 15/18 both with and without it, and mutating it to a flat weight broke no test. Removed rather than shipped.
