...
- [ ] **Open — routing relevance. Diagnosed and measured; blocked on a design decision, not on effort.** Against six queries with unambiguous ground truth the router retrieves the right file **0/6**. Two independent causes, both confirmed by measurement:
  - **`usageBoost()` can invent relevance.** `USAGE_COUNT_CAP` is 10 while a query-term match is worth 4, so a file matching *nothing* in the query outranks one matching two terms. `routeAndCompress()` records usage for everything it selects and nothing records whether the selection was correct, so the boost feeds on the router's own output. Measured here: `examples/test-dashboard.tsx` reached count 318 and won `"dashboard escapeHtml crashes"` on a single generic path match, beating `src/dashboard.js`, which matched two terms including the rare one.
  - **Scoring never reads file content.** The haystack is `path + owner + symbols + imports`. Symbol extraction is better than assumed (`escapeHtml` *is* indexed), but a metadata-only signal cannot separate `src/proxy.js` from `test/proxy.js` for a query about proxy behavior.
  - **Attempted and reverted, with numbers.** IDF term weighting (rare terms outweigh common ones) took noise from 16/23 to 13/26; adding a lower usage cap plus a relevance gate took it to 7/19. **Neither moved retrieval past 1/6**, and each broke existing tests — including one that explicitly asserts *"a.js should surface purely from its usage boost"*, i.e. the feedback loop encoded as a requirement, matching this roadmap's own v1.23.0 intent that usage "can outrank a cold keyword match". Weighting file identity above imports was also tried: no retrieval change, one file in nineteen of noise, so it was dropped as unmeasurable. All reverted; the tree is at v1.33.2.
  - **The decision needed first:** whether v1.23.0's intent stands. If usage may outrank a keyword match, the feedback loop is by design and retrieval cannot improve without a correctness signal to counterbalance it. If it may only break ties among already-relevant files, the cap and gate above are the fix and those two tests should change. That is a product call, not an implementation detail — which is why the code was reverted rather than shipped against a documented intent.
  - **Then:** content indexing (BM25 over file text, not metadata) is what actually addresses 0/6. That is a real piece of work, not a patch.
