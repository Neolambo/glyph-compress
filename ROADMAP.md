...
- [x] Current stable release is `v1.30.1`: documentation-only release, prompted by a repository-layout review against other mature open-source READMEs. Removed the long per-version changelog archive from README.md (now only in GitHub Releases/`RELEASE_NOTES.md`), and added a quick-nav row, a "When to Use / When to Skip" section, a "Compared to Alternatives" table, a comprehension-check "Proof" table, collapsible `<details>` for deep-dive sections, a root-level `llms.txt`, and `CODE_OF_CONDUCT.md`. No runtime/compressor behavior changed.
...
- [x] Root npm package, VS Code extension manifest, and VS Code extension lockfile are versioned to `1.30.1`.
- [x] npm `latest` is caught up to `1.30.0` (published directly by the maintainer, working around the OTP-confirmation flow this environment can't complete non-interactively); `1.24.0`/`1.25.0`/`1.27.0` were left unpublished as a harmless historical gap since `latest` already carries their fixes. `v1.30.1` publication is pending the same maintainer-driven `npm publish` step. VS Code Marketplace publication of `v1.13.0`-`v1.30.1` is still pending on maintainer action (needs a Marketplace PAT).
- [x] `npm test` passes 25 suites (unit, CLI, workspace, extension, proxy, metadata, snapshot, integration, holographic, intent, codebook-completeness, auto-level, cache-prefix-stability, tech-glyph-economics, code-minify-economics, context-router, mcp-server, team-codebook, logger, ast-spans, trust-warnings, npm-pack-smoke, adaptive-workspace-memory, anthropic-bridge, token-estimator-accuracy) for `v1.30.1`. `test/benchmark-alternatives.js` and the tokenizer-calibration/comprehension scripts remain deliberately excluded — reporting/manual tools, not pass/fail regression gates.
