...
- [x] Root npm package, VS Code extension manifest, VS Code extension lockfile, and `server.json` are versioned to `1.32.6`.
- [x] npm `latest` is caught up to `1.31.1` (published directly by the maintainer, working around the OTP-confirmation flow this environment can't complete non-interactively); `1.24.0`/`1.25.0`/`1.27.0` were left unpublished as a harmless historical gap since `latest` already carries their fixes. `v1.32.6` publication is pending the same maintainer-driven `npm publish` step. VS Code Marketplace is caught up to `v1.31.1`, published directly by the maintainer through the web management portal (marketplace.visualstudio.com/manage) rather than the CLI/PAT flow; `v1.32.6` publication there is still pending.
- [x] `npm test` passes 30 suites (unit, CLI, workspace, extension, proxy, metadata, snapshot, integration, holographic, intent, codebook-completeness, auto-level, cache-prefix-stability, tech-glyph-economics, code-minify-economics, context-router, mcp-server, team-codebook, logger, ast-spans, trust-warnings, npm-pack-smoke, adaptive-workspace-memory, anthropic-bridge, token-estimator-accuracy, context-budget-planner, privacy-redaction, dashboard, proxy-provider-inference) for `v1.32.6`. `test/benchmark-alternatives.js` and the tokenizer-calibration/comprehension scripts remain deliberately excluded — reporting/manual tools, not pass/fail regression gates.
