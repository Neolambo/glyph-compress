...
- [x] `v1.36.0`: **Context Router retrieval 2/6 -> 5/6, and the headline session number is now runnable on your own code.** Three independent reasons a correct answer was unreachable, none of which any suite could see, because the router returns a confident scored list whether or not the answer was ever a candidate. (1) Generated `.cjs` bundles were indexed beside their sources, splitting the evidence for a topic and then competing for the same slot — a query about workspace file ranking returned `src/workspace-intelligence.cjs` while the source missed the top three. Excluded by same-name `.js` sibling **or** by the bundler preamble, since the `vscode-ext/` bundles are built from `src/` and nothing beside them says generated. (2) **Class methods were not extracted at all** — extraction required `function`/`class`/`const` in front of a name, so for anything class-shaped the index held imports and constants and nothing the class does; `vscode-ext/glyph-middleware.js` was unreachable by any query about the privacy firewall or the decay zones. (3) The per-file symbol cap was 20 and symbols arrive in file order, so it kept the top of a file rather than a sample; now 120. Scoring also weights **where** a term matched — defines (5) beats named-after (3) beats imports (2), strongest field wins — because `test/privacy-redaction.js` was outranking the implementation on two filename matches. Noise 16/18 -> 13/18. Reproduce with `npm run measure:routing -- --verbose`. **The remaining miss is left alone:** "attentional decay compaction zones" names a concept living in prose, not in any symbol; reaching it means indexing comments, a different feature with its own noise. Tuning further against six queries would be fitting the benchmark. Six mutations, all caught — two of the tests did not bite when first written (a fixture with too few symbols for any cap to matter; an implementation winning on the alphabetical tie-break rather than on weighting) and were rebuilt until breaking the code broke them.
- [x] `v1.36.0`: **`glyph-compress measure <file>` — the session figure, on the user's own code.** Every number this project publishes came from one codebase and mostly one synthetic fixture, which is enough to demonstrate a mechanism and not enough to tell anyone what it does for theirs. The command simulates what dominates a real IDE session — the same file re-attached to every request while the conversation grows — and reports tokens **sent** and tokens **billed**, raw against compressed. On this repository's `src/compressor.js` at 10 turns: 283,525 -> 57,472 sent (-79.7%), 167,547 -> 34,304 billed (-79.5%). On `README.md` the two columns separate — -51.6% sent but only -25.3% billed — because compressing a growing history breaks the byte prefix on all 5 requests, which is why both columns are printed. Refusals matter more than the arithmetic on a reporting surface: a single turn is rejected rather than measured (nothing has repeated yet, so any percentage would be compression wearing a session-economics label), empty input is rejected rather than reported as 0%, and without `js-tiktoken` the output says the numbers are estimates. The engine is shared with `measure:differential` rather than duplicated.
- [x] `v1.36.0`: **two headline numbers made reproducible, and one of them was stale.** `measure:differential` is new — every other script attaches the file once, the one case with nothing to elide. At 10 re-attachments: 193,435 -> 41,182 tokens sent (-78.7%), 114,312 -> 24,526 billed (-78.5%) on OpenAI; -81.1% / -80.7% on Anthropic. `measure:cache` now prices **both** breakpoint policies on the same session instead of describing the old one in a closing paragraph, and doing so corrected a published figure: the **-41.6%** recorded for v1.33.6 does not reproduce on today's code, where the same comparison is **-32.9%** at 42 turns (coverage 82% -> 100%, 9,059 full-price tokens rather than 20,763). The v1.33.6 number was true when taken; v1.33.9 changed which copy of an elided block survives, which changed the session it was measured against. The shape holds: the gain grows without bound with session length, worst short-session case +0.2% at 4 turns. Release notes keep their original, correctly-dated numbers; what changed is that the current ones are printed by a command.
- [x] `v1.36.0`: **the economics guards mutation-tested — 8 mutations, 6 caught, 2 real gaps closed, 1 equivalent mutant recorded.** M6 (`definitionCost` forced to 0, so a word never repays the codebook entry introducing it) survived: nothing checked that a word appearing twice is rejected while the same word appearing often is admitted. Now asserted in both directions, so the test cannot be satisfied by a blanket length ban. M4 (the tokenizer-free estimate `floor(length / 8)` doubled to `/ 4`) survived in **every** configuration including the no-optional-deps job built to exercise it — and the first test written for it also passed under the mutation, because it re-derived the formula inside the test instead of calling the code. The formula now lives in `real-token-counter.js` as `conservativeWordTokens`, exported so the assertion reaches the same function the middleware uses: across 2,688 identifiers the estimate must never exceed the real count; doubled, it over-estimates 707 of them. M5 (`freq >= 2` relaxed to `>= 1`) survives and **stays** — at freq 1 the saving is `-2g-1`, negative for every glyph and word size, so no test can distinguish the two versions. Recorded so the next person does not hunt a counterexample that does not exist.
- [x] Current stable release is `v1.35.1`: **OpenAI measured — the three-provider table is complete, and the explanation given in v1.34.2 was wrong.** `anthropic` 3-4/4 with codewords, `openai` 4/12, `gemini` 1-2/4: only Anthropic sustains them, and the other two fail identically by resolving the reference then answering in the compressed vocabulary. v1.34.2 blamed Gemini's API shape — no system role in `v1beta generateContent`, so the codebook lands in the user turn — which is refuted: OpenAI *has* a system role, receives the codebook there, and fails the same way. The difference is **instruction-following, not transport**, and the wrong explanation would have justified restructuring Gemini requests for no gain. No behaviour change: `openai` was already on `§N` as the conservative default and stays there, now on evidence.
- [x] `v1.35.0`: **the codeword strategy is chosen per provider, from measurement.** The compressor already resolves a provider profile, so the choice lives there rather than in a global flag: **anthropic on** (comprehension tie, 8.2% fewer input tokens, 33 dictionary entries against 4), **gemini off** (1-2/4, never higher — it resolves the reference then answers `lagoon` instead of the class), **openai/raw/local off** as unmeasured. Verified against the real APIs with no flag passed: Anthropic picks codewords and scores 4/4 three times, Gemini picks `§N` at 3-4/4. An explicit option still overrides, and the strategy follows a per-call provider change so a compressor retargeted via `compressText(text, provider)` cannot send one provider's codewords to another. Fixing the harness to honour defaults exposed that it had been passing an explicit `codewordDictionary: false`, which overrode the profile and made it report `§N` for Anthropic while claiming to test the defaults.
- [x] `v1.34.2`: **word codewords do not survive Gemini — the experiment stays opt-in, and the default-flip item is closed as rejected on evidence.** Four checks per run, ~6 runs per cell: Anthropic `haiku-4-5` scores 3-4/4 with either form (a tie once variance is counted — earlier runs read 12/12 vs 10/12, later ones 7/8), while Gemini `2.5-flash-lite` scores 3-4/4 with `§N` and **1-2/4 with codewords, never higher**. Gemini's failure is diagnosed, not assumed: it answers `lagoon` and `dune`, which ARE the correct classes, resolving the reference and then replying in the compressed vocabulary — the same gap v1.34.1 fixed on Anthropic with `OUT: decode before answering`, which Gemini ignores, plausibly because `v1beta generateContent` has no system role and the codebook lands in the user turn. Moving that instruction to the end and making it imperative lifted Gemini 3/12 -> 6/12 and dropped Anthropic 12/12 -> 9/12: **no single placement serves both**, so it was reverted. The token argument stands — an ordinary word costs 1 token where `§N` costs 2 — but comprehension is a precondition, not a tradeable quantity. Still available for Anthropic-only deployments, where it is a tie on understanding and a win on cost (8.2% fewer input tokens, 20 dictionary entries against 3). **the codebook never told the model how to answer.** Running the codeword comparison against Anthropic (`claude-haiku-4-5`, 12 runs) found that on a dictionary-filling payload both modes returned the right *entities* in the wrong *vocabulary*: `§N` answered "13 classes (including §1, §2, §3 placeholders)" — counting markers as entities — and codewords answered "the class whose zebra is 7 is **lagoon**", where `lagoon` IS the correct class. Neither is a comprehension failure; the preamble said `Respond normally` in one variant and nothing in the other. Both now carry `OUT: decode before answering, and answer in the ORIGINAL names — never echo a code back.` Word codewords go **8/12 -> 12/12**; `§N` unchanged at 10/12. On that scenario codewords are now better or equal on every axis: 12/12 vs 10/12 comprehension, 20 vs 3 dictionary entries, 2,120 vs 2,310 input tokens (-8.2%). **Method note:** the first run alone showed 4/4 vs 2/4 and two repeats inverted it — a single sample was noise, and the real finding came from diagnosing *what* failed rather than counting passes.
- [x] **Closed — OpenAI measured (v1.35.1): 4/12 with codewords against 10/12 with `§N`. Codewords are an Anthropic-only strategy.** ~~Flip `codewordDictionary` to default, pending OpenAI and Gemini~~ — **closed, rejected on evidence:** The Anthropic evidence is one provider, one model, one scenario, n=12: enough to say the idea works and is directionally better, not enough to change a default for every user on three providers. `OPENAI_API_KEY=... node test/comprehension-check-codewords.js --codewords` and the Gemini equivalent are the missing arms. If they hold, codewords become the default and `§N` retires to a fallback for payloads that exhaust the vocabulary.
- [x] `v1.34.0`: **the cheapest codeword does not look like a code — `codewordDictionary: true`, opt-in.** BPE is a learned variable-length code trained on natural text, so it already gives ordinary words the shortest encodings; anything that *looks* like a code sits outside that distribution and costs more. Measured in running text: `rgb(...)` 8 tokens, `#A3F2C1` 7, `#A3F2` 5, base64-4 3, `§1`/emoji/CJK 2, **an ordinary English word 1**. A colour map — the intuition that started this — is the worst option measured. Halving the codeword moves the break-even, which is what matters: most identifiers are 2 tokens (`AuthenticationManager`, `handleSubmit`) and can never pay against a 2-token `§N`. Measured through the real pipeline at `aggressive`: entries go 4-10 -> 33-49 and savings improve +139/+224/+251/+201 real tokens across four files. Guarded by a vocabulary chosen against plausible collisions (excluding `salt`, `bloom`, `bean`, `leaf`, `spring`, `sage`, `delta`, `vector`, `matrix`) plus per-payload substring withdrawal, case-insensitive, so `zebraCrossing` disqualifies `zebra`. **Not verified: whether models decode a word codeword as reliably as `§N`** — that needs real API calls, no key was available, and it is why the flag is off. All three comprehension checks take `--codewords` and report the mode.
- [ ] **Open — run the codeword comprehension comparison.** `ANTHROPIC_API_KEY=... node test/comprehension-check-anthropic.js --codewords` against the same script without the flag, on all three providers. If comprehension holds, `codewordDictionary` should become the default and `§N` retire to a fallback for payloads that exhaust the vocabulary. If it does not, the flag should be removed rather than left as a trap — a saving that costs comprehension is not a saving.
- [x] `v1.33.10`: **the never-inflate guarantee did not hold in the shipped build — corrects v1.33.8.** That release claimed it held with or without the optional tokenizer; measured on the packaged artifact, where `js-tiktoken` is absent by design (a VSIX ships no node_modules), it was still inflating **+5.54% via compressMessages** and +0.15% via compressText. Every measurement backing v1.33.8 had been taken in the development tree, where the dependency IS installed — the verified path and the shipped path were not the same path. Cause: v1.33.8 gave `raw` a bare `compressed <= original` check, which is correct against a real count and wrong against an estimated one, since the estimator overstates improvement by 10-14% and the margin is exactly what stands in for the tokenizer when it is absent. Raw's permissive rule now applies only on the measured path. The guarantee is now stated in two parts rather than overclaimed: **exact** with js-tiktoken, **bounded by the estimator** without it (measured worst case +0.12%, raw/ultra). Both are asserted in `test/npm-pack-smoke.js`, which took three attempts — the first judged the output with the estimator itself and passed with the bug restored, the second used a fixture the bug did not appear on. The working version counts the packaged artifact's emitted bytes in the parent process with js-tiktoken; restoring the bug fails it with `+182 real tokens (5.54%) at src/proxy.js raw/light`.
- [x] `v1.33.9`: **differential transmission now keeps the cache prefix too — billed session cost on a re-attached file over 10 turns -63.7% -> -75.4% (OpenAI), -66.8% -> -78.5% (Anthropic).** The elision (v1.33.0) kept the NEWEST copy, so every turn rewrote the bytes of earlier turns and a provider's prefix cache keys on exactly those: measured **9 prefix breaks out of 9** over a 10-turn session. Keeping the **oldest** transmits identically (-75.4% either way, which also re-confirms the original -72.4% under js-tiktoken rather than the heuristic) and leaves history untouched — 1 break out of 9 — so the entire difference lands on the bill. It is only safe when nothing rewrites history: with Attentional Decay on, turns 0-5 come back with their code replaced by structural summaries, so a backwards marker would dangle (the `◈₍1₎` failure class of v1.32.6). v1.33.0's choice was correct for the case it was designed around. The surviving copy is now oldest when decay is off (the default), newest when on, and the marker wording follows the direction. Both directions pinned by their own test; neither mutation is caught by the other's.
- [x] `v1.33.8`: **compression is priced in real tokens, and the never-inflate guarantee is scoped to a SESSION.** Dictionary admission counted characters (`freq * (word.length - 2) - (word.length + 2)`), the wrong unit and wrong in the direction that approves losing substitutions: `§N` always costs 2 tokens while BPE merges `amount`/`validated`/`currency` into 1 each, so substituting them doubled the cost. Measured: characters -33% while real tokens **+37.8%**. The net-negative guard missed it because its two heuristic inputs err in opposite directions (+42.9% plain vs -24.1% glyph). Length cannot fix it — `AuthenticationManager` is 21 chars and 2 tokens, `processTransaction0` is 19 chars and 3 — so `js-tiktoken` is now an **optional** dependency: present, admission and the gate use real counts; absent, a conservative `chars/8` rule that never over-estimates. Also closed: `raw` skipped the guard (+0.5% on README.md), and the 10% margin no longer applies to real counts, where it was rejecting genuine savings. The cache-stable codebook header (437 real tokens vs the filtered 32, up to +50% on one call) is exempt, but only with assistant history **and** a provider that has a prefix cache — that bet returns up to -41.6% of session cost (v1.33.6), while `raw`/`local` have no cache to return it. Verified at session level: OpenAI/Gemini implicit caching 0.0% inflation and 0 prefix truncations across 2-42 turns; Anthropic 100% prefix coverage. `npm run benchmark` unchanged at 26%. Reproduce with `npm run measure:implicit-cache`, `npm run measure:cache`, `npm run measure:showcase`.
- [x] `v1.33.7`: **security — the privacy firewall was bypassed on every net-negative fallback.** Both entry points fall back to the original payload when compression would cost more than it saves, and both returned the **raw** original rather than the redacted one: `compressText()` returned `text` instead of `safeText`, `compressMessages()` returned the untouched input array. The firewall ran, the redactions were computed, the source map recorded them, the API reported the firewall as active — and the fallback discarded the filtered text. Reachable on released code with ordinary input, and the two conditions coincide rather than being independent: short messages are where compression does not pay *and* where people paste a credential to ask about it. Hidden because the redaction suite only exercised payloads that compress successfully. Both paths now return the filtered original; covered for both entry points, each asserting the fallback branch was actually taken before asserting anything about it.
- [x] `v1.33.6`: **the Anthropic cache breakpoint was on the wrong axis — full-price tokens in a 42-turn session 20,763 -> 0, effective cost -41.6%.** Prompt caching is prefix-based, so `cache_control` belongs at the *end* of what was sent; GlyphCompress marked the **largest user block**, which is almost always the file attached at the start and does not move as the conversation grows. Every later turn fell outside the cached prefix and was billed in full on every request. Coverage at the final turn: 8 turns 96% -> 100% (-1.4%), 18 turns 88% -> 100% (-13.8%), 42 turns 74% -> 100% (-41.6%). Short sessions do not regress — worst case measured is +0.2% at 4 turns, bounded by the 0.25x write premium on one turn while the gain is unbounded in length. Keeping the old marking as a second "stable floor" breakpoint was measured and rejected: identical numbers, and with prefix caching any change that invalidates the head invalidates the floor too. Reproduce with `npm run measure:cache`.
- [x] `v1.33.5`: **two published numbers retracted, and a harness so it cannot recur.** No behavior change. The retrieval figures in v1.33.3 and v1.33.4 were produced by an ad-hoc script run against the working tree, where ranking gives **+3 to any git-dirty file** — including `src/workspace-intelligence.js`, the target of one of the six queries and the file being edited. v1.33.3's real delta is **1/6 -> 2/6**, not 0/6 -> 3/6, and its claim that both halves are load-bearing is withdrawn: measured cleanly, either alone achieves the full effect. v1.33.4's table shifts by one point with its conclusion intact. `npm run measure:routing` now refuses to run on a dirty tree and pins the usage history rather than inheriting it; `--seed-usage` rebuilds the feedback loop deterministically (`examples/test-dashboard.tsx` reaches count 312, against the 318 recorded before the original data was lost — independent confirmation of the diagnosis, which was never in question).
- [x] `v1.33.4`: **two silent failures, found while measuring something else.** (1) `readTextFile()` returned `''` for any file over `maxFileBytes` instead of reading a prefix, so an oversized file contributed no symbols, no imports and no diagnostics and could never be routed to — while the router still returned a confident scored list without it. On this repository exactly one file crossed the 120,000-byte default: `vscode-ext/glyph-middleware.js` at 122,875 bytes, the ESM source of truth holding the compressor, the privacy patterns and the decay zones, indexing as **0 lines, 0 symbols**. Now 2,824 lines and 20 symbols; a query for a symbol unique to it went from unranked to rank 1. (2) The `test()` helpers in `test/context-router.js` and `test/integration.js` called `fn()` inside a `try/catch` — an async test rejects *after* fn returns, so three tests printed a green tick and reported "0 failed" with guaranteed-false assertions. Verified by forcing one. `integration.js` was worse: it ends in `process.exit()`, which tore down the process before the rejection could even surface as an unhandled one.
- [x] `v1.33.3`: **usage stopped inventing relevance in the Context Router — retrieval 0/6 -> 3/6.** v1.23.0 let a file's past-selection boost "outrank a cold keyword match". That is unsound here, because `routeAndCompress()` records usage for everything it selects and *nothing* records whether the selection was correct, so the boost feeds on the router's own output: `examples/test-dashboard.tsx` reached count 318 and won `"dashboard escapeHtml crashes"` on one generic path match, beating `src/dashboard.js`, which matched the rare term. Usage now breaks ties among files that already matched and cannot manufacture a match (gate), and `USAGE_COUNT_CAP` drops 10 -> 3, below the 4 points one term match earns (cap). **Figures corrected in v1.33.5** — the originals were taken on a dirty working tree, where the file under edit (the target of one query) collected the +3 git-dirty ranking boost. Measured cleanly with a seeded usage history: **1/6 -> 2/6 retrieval, noise 17/18 -> 16/18**. With no usage history every revision scores 2/6, so the fix repairs what the feedback loop was costing rather than adding capability. The two halves are **redundant, not complementary** — either alone achieves the full effect; both are kept on a design argument, each with its own unit test. Reproduce with `npm run measure:routing -- --seed-usage`. IDF term weighting was implemented alongside and **dropped**: it changed neither retrieval nor noise and no test caught its removal.
- [x] `v1.33.2`: **cross-session determinism documented and guarded.** A compressed body is cacheable across sessions only if identical input yields identical bytes — by default it does not, because `§N` indices follow session learning order (`const §1` fresh vs `const §36` warmed). Setting `workspacePath` fixes it via the v1.13.0 dictionary cache. Measured across all four combinations: the team registry makes no difference on its own; `workspacePath` is the whole factor. An earlier conclusion crediting the team codebook was wrong and is corrected in the release notes.
- [x] `v1.33.1`: **the Context Router ran against a stale file index.** `selectRelevantFiles()` used a persisted codebook as-is, rebuilding only when none existed — so routing saw whatever the last `glyph-compress inspect` left behind. Measured here: a snapshot dated 2026-07-19 listed 119 files where a rebuild finds 136, leaving 17 invisible to routing including `src/anthropic-bridge.js`. Fixed by seeding an incremental rebuild with the cached copy; measured cost none (337ms vs 346ms per call).
- [ ] **Open — on OpenAI and Gemini, compressing a multi-turn session currently costs more than not compressing. Measured, reproducible with `npm run measure:implicit-cache`.** This is the most serious open finding in the project and it needs a decision, not just an implementation.

  Anthropic caching is explicit — v1.33.6 fixed where the breakpoints go. OpenAI and Gemini cache **implicitly**: the provider matches the longest identical *byte* prefix against what it has already seen. There is no marker to place; the only thing that matters is whether turn N is byte-identical to turn N-1 for as long as possible. An uncompressed conversation satisfies this perfectly — measured here, **0 prefix truncations across 20 turns**.

  GlyphCompress re-compresses the entire history on every turn while the dictionary keeps learning, so the same original text serialises to different bytes each request. Measured: the first user message shrinks by exactly 16 characters per turn and diverges 78 characters further along each time. Truncations: **3/20 on OpenAI, 19/20 on Gemini** — several to zero shared bytes.

  | Provider | Turns | Tokens sent | Billed with cache |
  |---|---|---|---|
  | openai | 18 | +13.0% | **+51.7%** |
  | openai | 42 | +5.2% | **+21.8%** |
  | gemini | 18 | +31.5% | **+123.9%** |
  | gemini | 42 | +36.3% | **+150.6%** |

  Two independent problems are stacked here, and they have different fixes:
  - **The payload grows before caching is even considered.** On this fixture compression *adds* tokens: characters drop 33% (17,238 -> 11,473) while real js-tiktoken count rises **37.8%** (3,496 -> 4,818). `§N` glyphs cost more than the identifiers they replace. The injected codebook is not the culprit — measured at 105 tokens. The measured-loss gate that disabled `TECH_GLYPHS` substitution on all three providers does not appear to cover `§N` substitution in the `compressMessages` path; the same content through single-message `compressText` correctly declines to compress and returns 0.0%.
  - **Cache destruction on top of that**, which is the gap between the two columns above.

  **Caveats stated up front:** one fixture, deliberately identifier-repetitive because that is exactly what the dynamic dictionary targets — so it is the intended use case, not a hostile one. Other content behaves differently: `src/compressor.js` and `README.md` both measure 0.0% through `compressText`, i.e. the guard fires and the original is sent. The cached-input rate used is OpenAI's 0.5x; Gemini's implicit caching is nearer 0.25x, which makes the loss larger there, not smaller.

  **The decision needed:** whether the dynamic dictionary is compatible with implicit-cache providers at all. Freezing the dictionary after the first turn would restore prefix stability at the cost of compressing later turns less; gating `§N` by real measured token delta would fix the inflation but not the byte churn. Both are real changes to the core value proposition, which is why this is recorded rather than patched.

- [ ] **Open — routing relevance, second half. The remaining 3/6 all fail the same way: the test file outranks the source file.** `test/privacy-redaction.js` beats `vscode-ext/glyph-middleware.js` for "privacy redaction pattern for api keys"; `test/token-estimator-accuracy.js` beats `src/token-estimator.js`. This is not the usage bug fixed in v1.33.3 — it is the second cause, and it needs real work rather than a weight change:
  - **Scoring never reads file content.** The haystack is `path + owner + symbols + imports`. Symbol extraction is better than assumed (`escapeHtml` *is* indexed), but a metadata-only signal systematically favours the file whose *name* announces the topic over the file that *implements* it — a test is named after its subject, a 2,800-line implementation is not.
  - **Content indexing was built and measured and did not ship. Retrieval stayed at exactly 3/6, noise at 15/18, across five variants.** Recorded in full because the negative result is the useful part:
    ...
    It did improve *rank within* the top 3 (`src/dashboard.js` 2nd→1st, `src/workspace-intelligence.js` 3rd→1st), but no metric this project holds itself to measures that, and doubling-to-quintupling the codebook for an unmeasured effect is the trade v1.33.3 just refused for IDF.
  - **Why it failed, which is the part worth keeping.** Selecting which terms to *store* by tf·idf is the wrong objective. idf is built to score a query term, and it rewards terms unique to one file — so a 2,800-line file's stored vocabulary filled with one-off compound identifiers (`this._minifyreplace`, `addmatches`, `this.dynamiccounter`), each df=1 and therefore maximal idf. Nobody queries those. The words a person types — "privacy", "decay" — appear in a handful of files, so they score *lower* and never made the list. A df>1 filter did not rescue it either, because the committed `.cjs` build artifacts duplicate the sources and inflate df for every identifier in them.
  - **So the real blocker is upstream of scoring:** the index treats generated bundles (`src/workspace-intelligence.cjs`, `vscode-ext/*.cjs`) as peers of their sources. They compete for slots, split the term statistics, and show up as noise in every result above. Excluding build outputs from the codebook is the prerequisite; retrying content indexing before that is retrying it against corrupted df.
  - **Refuted, recorded so it is not retried:** IDF term weighting in *scoring* (v1.33.3) — retrieval and noise identical with and without, no test caught its removal. Content indexing in *selection* (v1.33.4) — the table above.
- [x] `v1.33.0`: **trasmissione differenziale — the largest measured saving this project has shipped**, and it comes from not sending things twice rather than compressing harder. IDEs re-attach open-file context every turn; measured, the same file emitted 1635|1635|1592|1592|1592 real tokens over 5 turns — full weight every time. Eliding earlier duplicates and keeping the newest copy: **10,515 -> 2,909 tokens (-72.4%)** at `standard`, -67.9% at `aggressive`. Composes with prompt caching (which recovers 67% of the same repetition) rather than competing. Direction is the design: decay compacts OLD turns, so the newest copy is the survivor and a decayed turn holds only a marker.
- [x] `v1.32.9`: **the legacy engine exported from the package root emitted glyphs the model was never given a definition for** — 6 of them, including the `₍` `₎` subscript delimiters of the `◈₍1₎` file-reference notation, the most load-bearing construct in the output. Root cause was three drifts in one function: `SYM:` was a hand-written string covering 9 of `STRUCTURE_GLYPHS`' 21 entries (sitting directly beneath a comment describing the identical drift fixed for `TECH_GLYPHS` in v1.16.0), `ERROR_CODES` was imported and never rendered, and the `₍N₎` notation was only implied. `test/codebook-completeness.js` never covered this engine at all. Now generated from the tables and covered.
...
- [x] Root npm package, VS Code extension manifest, VS Code extension lockfile, and `server.json` are versioned to `1.33.2`.
- [x] npm `latest` is caught up to `1.31.1` (published directly by the maintainer, working around the OTP-confirmation flow this environment can't complete non-interactively); `1.24.0`/`1.25.0`/`1.27.0` were left unpublished as a harmless historical gap since `latest` already carries their fixes. `v1.33.2` publication is pending the same maintainer-driven `npm publish` step. VS Code Marketplace is caught up to `v1.31.1`, published directly by the maintainer through the web management portal (marketplace.visualstudio.com/manage) rather than the CLI/PAT flow; `v1.33.2` publication there is still pending.
- [x] `npm test` passes 30 suites (unit, CLI, workspace, extension, proxy, metadata, snapshot, integration, holographic, intent, codebook-completeness, auto-level, cache-prefix-stability, tech-glyph-economics, code-minify-economics, context-router, mcp-server, team-codebook, logger, ast-spans, trust-warnings, npm-pack-smoke, adaptive-workspace-memory, anthropic-bridge, token-estimator-accuracy, context-budget-planner, privacy-redaction, dashboard, proxy-provider-inference) for `v1.33.2`. `test/benchmark-alternatives.js` and the tokenizer-calibration/comprehension scripts remain deliberately excluded — reporting/manual tools, not pass/fail regression gates.
