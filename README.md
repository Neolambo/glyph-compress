...
This project reports honest numbers, not just best cases — so here's the direct answer on fit, backed by the measurements in [📏 Benchmark Snapshot](#-benchmark-snapshot-v1352) and [🧪 Realistic Benchmark Notes](#-realistic-benchmark-notes) below.
...
### New in v1.35.2 (Documentation realigned with the code)

An audit of README against the source, treating the code as the only source of truth, found **seven discrepancies** — all now corrected. No documented-but-unimplemented feature was found; the drift was in figures superseded by recent releases and in omissions from accretion.

The headline one: this README claimed **22%** aggregate savings in three places while `npm run benchmark` reports **26%**. The 22% predates the measured-loss gating and the v1.33.x economics fixes, which removed losing substitutions and therefore *raised* the aggregate — the project was under-reporting its own result.

Also corrected: `--target`/`--target-api-url` were missing from the CLI table though present in `--help`; the net-negative fallback was described with pre-v1.33.8 semantics; `codewordDictionary` was still marked "off by default" after v1.35.0 made it per-provider; the Project Structure omitted 6 of 14 `src/` files; there was no Requirements section (Node 18+, and `zod` was never mentioned); and the VSIX limitation — no `js-tiktoken`, so 1 dictionary entry against 33 — was undocumented.

### Also recent (v1.35.1 — OpenAI measured, the table is complete)

| Provider | `§N` | Word codewords |
|---|---|---|
| anthropic `haiku-4-5` | 3–4/4 | **3–4/4** |
| openai `gpt-4o-mini` | 10/12 | **4/12** |
| gemini `2.5-flash-lite` | 3–4/4 | **1–2/4** |

**Only Anthropic sustains codewords.** The other two resolve the reference correctly and then answer in the compressed vocabulary, ignoring the instruction to expand back.

This also corrects v1.34.2, which blamed Gemini's lack of a system role. OpenAI has one, receives the codebook there, and fails identically — the difference is instruction-following, not transport. No behaviour change: `openai` was already defaulted to `§N`, now on evidence rather than caution.

### Also recent (v1.35.0 — each provider gets the strategy it was measured with)
...
Anthropic is a tie once variance is counted, with codewords costing 8.2% fewer input tokens. Gemini is not a tie and not noise: six runs, never above 2/4. It resolves references correctly and then answers in the compressed vocabulary — `lagoon` instead of `RefundEligibilityValidator` — ignoring the codebook's instruction to expand back, plausibly because Gemini has no system role and the codebook lands in the user turn.

Repositioning that instruction helped Gemini (3/12 → 6/12) and hurt Anthropic (12/12 → 9/12). No single placement serves both, so the finding was recorded rather than shipped. The token argument still holds — an ordinary word costs 1 token where `§N` costs 2 — but comprehension is a precondition, not a tradeable quantity.

### Also recent (v1.34.1 — the codebook never told the model how to answer)
...
The reasoning is that BPE is itself a learned variable-length code trained on natural text, so it already gives ordinary words the shortest encodings. Anything that *looks* like a code is outside that distribution and costs more:

| Codeword | Real tokens |
|---|---|
| `rgb(163,242,193)` | 8 |
...
**Corrects v1.33.8**, which stated the never-inflate guarantee held with or without the optional tokenizer. It did not: measured on the packaged artifact, where `js-tiktoken` is absent by design because a VSIX ships no `node_modules`, compression was still inflating by **+5.54%**. Every measurement backing that claim had been taken in the development tree, where the dependency *is* installed — the verified path and the shipped path were not the same path.

The guarantee is now stated in two parts rather than overclaimed:

| | Guarantee |
|---|---|
| With `js-tiktoken` (npm default) | **Exact** — never more tokens out than in |
| Without it (VSIX, `--no-optional`) | **Bounded by the estimator**, measured worst case **+0.12%** |

Both bounds are asserted against the real packaged artifact.

### Also recent (v1.33.9 — differential transmission now keeps the cache prefix too)

**Billed session cost on a re-attached file over 10 turns: −63.7% → −75.4% on OpenAI, −66.8% → −78.5% on Anthropic.** Not one token less is transmitted — the entire gain is prompt cache that was being thrown away.

Eliding a file repeated across turns kept the *newest* copy, which rewrites the bytes of earlier turns on every request — and a prefix cache keys on exactly those bytes. Measured: **9 prefix breaks out of 9** over ten turns. Keeping the *oldest* copy transmits identically (−75.4% either way) and leaves history untouched: **1 break out of 9**.

It is only safe when nothing rewrites history. With Attentional Decay enabled old turns are compacted, so a backwards reference would point at code that had been summarised away — which is why v1.33.0 chose the newest, correctly, for that case. The surviving copy is now the oldest when decay is off (the default) and the newest when it is on, with the marker wording following the direction.

### Also recent (v1.33.8 — compression is priced in real tokens, over a session)

**GlyphCompress no longer sends more tokens than it received**, guaranteed across a session of ordinary use rather than per individual call.

Dictionary admission used to count **characters**, which is the wrong unit and wrong in the direction that approves losing substitutions: a `§N` glyph always costs 2 real tokens while BPE merges `amount`, `validated` or `currency` into 1 each. Measured on identifier-repetitive source, characters fell 33% while **real tokens rose 37.8%**. Length cannot separate the cases — `AuthenticationManager` is 21 characters and 2 tokens, `processTransaction0` is 19 characters and 3 — so **`js-tiktoken` is now an optional dependency**: installed, the decision uses real BPE counts; absent, a conservative length rule keeps the guarantee and simply saves less.

The scope matters. The cache-stable codebook header costs 437 real tokens against 32 for the filtered form, so one call carrying it can measure +50% — while buying byte-identical prefixes worth up to **−41.6% of session cost**. It stays, but only when a session is actually in progress *and* the provider has a prefix cache to amortise it into.

Verified at session level: **0.0% inflation and 0 prefix truncations** across 2–42 turns on OpenAI and Gemini, **100% prefix coverage** on Anthropic, benchmark unchanged at 26%. Reproduce with `npm run measure:implicit-cache`, `npm run measure:cache` and `npm run measure:showcase`.

### Also recent (v1.33.7 — security: the privacy firewall was bypassed on every fallback)
