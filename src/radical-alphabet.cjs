...
var radical_alphabet_exports = {};
__export(radical_alphabet_exports, {
  ...
  REVERSE_COMPOSITIONS: () => REVERSE_COMPOSITIONS,
  REVERSE_DOMAIN: () => REVERSE_DOMAIN,
  REVERSE_ERRORS: () => REVERSE_ERRORS,
  REVERSE_TECH: () => REVERSE_TECH,
  STRUCTURE_GLYPHS: () => STRUCTURE_GLYPHS,
  TECH_GLYPHS: () => TECH_GLYPHS,
  getAlphabetStats: () => getAlphabetStats
});
module.exports = __toCommonJS(radical_alphabet_exports);
var RADICALS = {
  // Domain radicals
  CODE: "\u2E80",
  ...
  CHAIN: "\u2E8C",
  // connection / integration / API
  HAMMER: "\u2E8D",
  // build / create / construct
  LENS: "\u2E8E",
  // analyze / inspect / review
  ARROW: "\u2E8F"
  // deploy / ship / transfer
};
var DOMAIN_GLYPHS = {
  frontend: "\u25C8",
  ai_ml: "\u25C9",
...
function getAlphabetStats() {
  return {
    ...
    totalSymbols: Object.keys(RADICALS).length + Object.keys(DOMAIN_GLYPHS).length + Object.keys(ACTION_GLYPHS).length + Object.keys(TECH_GLYPHS).length + Object.keys(STRUCTURE_GLYPHS).length
  };
}
