...
// Same measurement, extended to Gemini: [wordTokens, glyphTokens], captured
// live against the real `models/{model}:countTokens` API (gemini-2.5-flash-
// lite) — Gemini has no offline pure-JS tokenizer equivalent to js-tiktoken,
// so unlike the OpenAI table above this one can only be regenerated with a
// real API key (see test/tokenizer-calibration-gemini.js, dev-only/manual,
// not part of `npm test`). Same finding as OpenAI: 26/28 TECH_GLYPHS and
// 32/33 code keywords are a net token loss on Gemini too — only `csharp`
// and `nextjs` glyphs win (their words are 2 tokens, the glyphs 1).
const MEASURED_TECH_GLYPH_TOKENS_GEMINI = {
  typescript: [1, 1], javascript: [1, 2], python: [1, 1], rust: [1, 1],
  go: [1, 1], java: [1, 1], csharp: [2, 1], swift: [1, 1], ruby: [1, 1],
  react: [1, 3], nextjs: [2, 1], vue: [1, 4], angular: [1, 1], svelte: [1, 1],
  django: [1, 1], rails: [1, 1], express: [1, 2], fastapi: [2, 4], docker: [1, 1],
  kubernetes: [1, 4], terraform: [1, 4], postgres: [1, 1], mysql: [1, 3],
  mongodb: [1, 3], redis: [1, 3], llm: [2, 3], agent: [1, 1], prompt: [1, 1],
};

const MEASURED_CODE_KEYWORD_TOKENS_GEMINI = {
  return: [1, 1], function: [1, 1], const: [1, 1], let: [1, 1],
  import: [1, 1], export: [1, 1], def: [1, 1], class: [1, 1],
  from: [1, 1], yield: [1, 1], 'self.': [2, 2],
  int: [1, 2], void: [1, 2], char: [1, 2], float: [1, 2],
  double: [1, 2], long: [1, 2], short: [1, 2],
  fn: [1, 1], pub: [1, 1], mut: [1, 1], impl: [1, 1],
...
class GlyphCompressor {
  ...
  _elideRepeatedBlocks(messages, rolesToCompress) {
    const FENCE = /```[\s\S]*?```/g;
    const occurrences = new Map(); // block text -> [{ msgIndex }]
    ...
    // Only the ones that actually repeat, and only where the marker is
    // cheaper than the block it replaces.
  ...
  _compressMessagesForStrategy(messages, provider, origTokens, baseState, candidate) {
    ...
    const allCompressibleText = messages
      ...
      // Elision markers are metadata addressed to the model, not user
      // content, so they must not feed the dynamic dictionary. Left in, the
      // marker repeats once per elided turn, its words clear the freq>=2 bar,
      // and the instruction dissolves into "[§40 §41 later in this §34 — see
