...
test('Provider profiles: tune dynamic dictionary thresholds by provider', () => {
  ...
  assert(raw.sourceMap.version === '1.10.0', 'Should include v1.10.0 source map version');
...
test('Source maps: expose reversible dictionaries', () => {
  ...
  assert(r.sourceMap.version === '1.10.0', 'Should include source map version');
...
test('Source maps: CommonJS root export matches ESM behavior', () => {
  ...
  assert(r.sourceMap.version === '1.10.0', 'Should expose source maps through require()');
...
test('CLI: source-map flag prints source map JSON', () => {
  ...
  assert(output.includes('"version": "1.10.0"'), 'Should print source map version');
...
test('Workspace intelligence: builds persistent codebook and ranks relevant files', () => withTempWorkspace((dir) => {
  ...
  assert(codebook.version === '1.10.0', 'Should use v1.10.0 codebook schema');
...
test('Workspace intelligence: CLI inspect prints JSON summary', () => withTempWorkspace((dir) => {
  ...
  assert(result.version === '1.10.0', 'Should print v1.10.0 inspect output');
