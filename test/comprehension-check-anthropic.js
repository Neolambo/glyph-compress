 ...
 * -------------------------------------------------------------------------
 * GlyphCompress — Anthropic Comprehension Spot-Check (dev-only, manual)
 *
 * Sibling to test/comprehension-check-gemini.js and
 * test/comprehension-check-openai.js — same scenario, same checks, so all
 * three are directly comparable across providers. See those files' headers
 * for the full rationale (ROADMAP.md's "Real Task Evaluation" item, one
 * scenario/one provider each, not a statistical benchmark).
 *
 * This is NOT a statistically meaningful benchmark and is NOT part of
 * `npm test` — it needs a real ANTHROPIC_API_KEY, network access, and
 * costs a small amount of real generation quota.
 *
 * Usage: ANTHROPIC_API_KEY=... node test/comprehension-check-anthropic.js [model]
 */
...
// Same scenario as the Gemini/OpenAI sibling scripts, so all three spot-
...
async function run() {
  ...
  if (!res.ok) {
    console.error('messages failed:', JSON.stringify(json, null, 2));
    process.exit(1);
  }
